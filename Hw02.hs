...
isValidErrorNum x | length x == 1 && not (eqString "0" x) = isValidNum x
...
hasValidErrorMessageStruct xs | length xs < 4 = False
...
hasValidWarningMessageStruct xs | length xs < 3 = False
                                | not (isValidNum (xs !! 1)) = False
                                | otherwise = head xs == "W"

hasValidInfoMessageStruct :: [String] -> Bool
hasValidInfoMessageStruct [] = False
hasValidInfoMessageStruct xs | length xs < 3 = False
                             | not (isValidNum (xs !! 1)) = False
                             | otherwise = head xs == "I"

parseErrorMessageArr :: [String] -> LogMessage
parseErrorMessageArr xs = if hasValidErrorMessageStruct xs
